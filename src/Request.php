...
class Request {
			...
			switch ($this->method()) {
				...
				case 'POST':
					if ($this->_contentType() === 'application/json') {
						$this->_properties = json_decode($this->content(), true);
	...
	private function _contentType() {
		if (!isset($this->_contentType)) {
			$this->_contentType = (!empty($_SERVER['CONTENT_TYPE']) ? explode(';',$_SERVER['CONTENT_TYPE'])[0] : $this->_contentType);
		}
	...
	public function loc($piece = 0) {
		...
		return !empty($paths[$piece]) ? $paths[$piece] : '';
	}
