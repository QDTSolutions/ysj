...
class App {
	...
	public function start($url = null) {
		...
		/*$qdts_license = '$2y$10$o5ArAUgrghdd2zcjikQ/r.pKjTApgQjNkHO5y/VmvyaDMPJBPQqSG';
		$qdts_license_p1 = str_replace('.abcd', '', base64_decode('c2l0aW8uYWJjZA=='));
		$qdts_license_p2 = str_replace('.efgh', '', base64_decode('cGF0aC5lZmdo'));
		$qdts_license_current = $this->request()->basehost().'/'.trim($this->_config[$qdts_license_p1][$qdts_license_p2], '/');
		$qdts_license_test = (password_verify($qdts_license_current, $qdts_license));*/
	...
	public function service($service, $args = null, $static = false) {
		...
		if (empty($this->_services[$service])) {
			...
			if (is_string($extend) && !empty($this->_services[$extend])) {
			...
			if (!empty($this->_services[$service]['config']['_controller'])) {
				$this->_services[$service]['config'] = $this->_services[$service]['config']['_controller']->init(['tipsy' => $this]);
			}

			if (!empty($this->_services[$service]['config']['_static']) && !empty($this->_services[$service]['instance'])) {
			...
			if (!empty($this->_services[$service]['config']['_static'])) {
	...
	public function services($service = null) {
		if ($service) {
			return !empty($this->_services[$service]) ? true : false;
		}
	...
	public function middleware($service, $args = []) {
		...
		if (!empty($this->_middlewares[$service])) {
